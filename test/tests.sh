...
httpd -v | grep -q "Server version: Apache\/${APACHE_VER}"
echo "OK"

httpd -M > /tmp/apache_modules
echo -n "Checking Apache modules... "

if ! cmp -s /tmp/apache_modules /home/anaxexp/apache_modules; then
    echo "Error. Apache modules are not identical."
    diff /tmp/apache_modules /home/anaxexp/apache_modules
    exit 1
fi
